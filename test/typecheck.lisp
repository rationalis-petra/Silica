...
(define-test typing
  ...
  (define-test type-check
    ;; int : τ
    (is α= int-type
        (sigil:check int-type τ +empty-env+))

    ;; int → int : τ
    (is α= (mk-arr int-type int-type)
        (sigil:check (mk-arr int-type int-type) τ +empty-env+))

    ;; (bot) ∀ a. a ◂ τ
    (is α= (mk-∀ 'a (mk-tvar 'a))
        (sigil:check (mk-∀ 'a (mk-tvar 'a)) τ +empty-env+))

    ;; (Id) λ α. α ◂ τ → τ
    (is α=
        (mk-tλ 'a τ (mk-tvar 'a))
        (sigil:check (mk-λ 'a (mk-var 'a)) (mk-karr τ τ) +empty-env+))
    ...
    ;; w : int
    (is α= (mk-val 2)
           (sigil:check (mk-val 2) int-type +empty-env+))

    ;; (λ x. 2) : int → int
    (is α= (mk-mλ 'x int-type (mk-val 2))
        (sigil:check (mk-λ 'x (mk-val 2))
                    (mk-arr int-type int-type)
                    +empty-env+))

    ;; (Λ a. λ x. x) : ∀ a. a → a
    (is α= (mk-abs 'a τ (mk-mλ 'x (mk-tvar 'a) (mk-mvar 'x)))
        (sigil:check (mk-abs 'a (mk-λ 'x (mk-var 'x)))
    ...
    (is α=
        (mk-app (mk-mλ 'x int-type (mk-mvar 'x))
                (mk-val 2))
        (sigil:check (mk-app (mk-λ 'x int-type (mk-var 'x))
                            (mk-val 2))
                    int-type
                    +empty-env+))
    ...
    (is α=
        (mk-sig (entries (mk-decl 'x int-type)
                      (mk-decl 'y int-type)))
        (sigil:check (mk-sig (entries (mk-decl 'x int-type)
                                  (mk-decl 'y int-type)))
                    τ
                    +empty-env+)))
  ...
  (define-test type-infer
    ...
    (is α=
        int-type
        (car (sigil:infer (mk-val 2) +empty-env+)))

    ;; infer int = τ
    (is α= τ (car (sigil:infer int-type +empty-env+)))

    ;; infer (λ x : int. x) = int → int
    (is α=
        (mk-arr int-type int-type)
        (car (sigil:infer (mk-λ 'x int-type (mk-var 'x)) +empty-env+)))

    ;; infer (λ x : int. x) 2 = int
    (is α=
        int-type
        (car (sigil:infer (mk-app (mk-λ 'x int-type (mk-var 'x)) (mk-val 2)) +empty-env+)))

    ;; infer (Λ a. λ x : a. x) = ∀ a. a → a
    (is α=
        (mk-∀ 'a (mk-arr (mk-tvar 'a) (mk-tvar 'a)))
        (car (sigil:infer (mk-abs 'a (mk-λ 'x (mk-var 'a) (mk-var 'x))) +empty-env+)))
  ...
  (define-test type-reduction
      (α= (sigil::ty-reduce int-type) int-type)

      (α= (sigil::ty-reduce (mk-tapp (mk-∀ 'a (mk-var 'a)) int-type)) 
          int-type)))
